  ...
  <page-container title="审核认定">
    <div>
      ...
      <el-table :data="filteredBreachList" style="width: 100%">
        ...
        <el-table-column prop="DefaultReason" label="违约原因" />
        ...
        <el-table-column label="审核状态">
          <template #default="scope">
            <el-select v-model="scope.row.Status" @change="UpdateState(scope.row)">
              <el-option label="未审核" value="未审核" />
              <el-option label="同意" value="同意" />
              <el-option label="拒绝" value="拒绝" />
            </el-select>
...
import { artGetBreachService, artGetReasonService, artDelService, artEditReviewService } from '@/api/article.js'
...
async function fetchBreachList() {
    try {
        ...
        if (breachResponse && breachResponse.data) {  
            ...
            // 初始化审核状态为“未审核”，并将原因ID转换为原因字符串
            breachList.value = breachResponse.data.data
        } else {
...
async function UpdateState(row) {
    console.log('更新审核状态:', row.Status);
    try {
        // 构建请求数据
        const data = {
            reviewer_id: userStore.user.userID, // 从 userStore 中获取 reviewer_id
            status: row.Status, // 当前审核状态
        };

        // 调用 API 服务
        const response = await artEditReviewService(row.ID,data);
...
function handleSearch() {
    ...
    filteredBreachList.value = breachList.value.filter(item => {
        ...
        const statusMatch = searchAuditStatus.value === '' || item.Status === searchAuditStatus.value;
